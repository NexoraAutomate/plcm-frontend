...
const TEMPLATE_CSV = [
  ...
  "Design,Initial design phase,projects",
  "Available,In stock and free,units",
  "Open,Order is open,orders",
].join("\n");
...
export function StatusImportButton({
  disabled = false,
  onImported,
}: StatusImportButtonProps) {
  ...
  const downloadTemplate = () => {
    ...
    a.href = url;
    a.download = "statuses-template.csv";
    a.click();
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setValidating(true);
    try {
      const res = await api.statuses.importSpreadsheet(next, true);
      setPreview({
  ...
  const handleImport = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await api.statuses.importSpreadsheet(file, false);
      const imported = res.data.imported ?? 0;
      const skipped = res.data.skipped ?? 0;
      const parts = [`Imported ${imported} status${imported === 1 ? "" : "es"}`];
      ...
      if (rowErrors) {
        ...
        toast.error(getErrorDetail(err) || "Failed to import statuses");
      }
    } finally {
      setSubmitting(false);
    }
    ...
    <>
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Import statuses</DialogTitle>
            <DialogDescription>
              Upload a CSV or Excel (.xlsx) file with{" "}
              <code className="text-xs font-mono bg-muted px-1 rounded">Status</code>,{" "}
              <code className="text-xs font-mono bg-muted px-1 rounded">Description</code>, and{" "}
              <code className="text-xs font-mono bg-muted px-1 rounded">Entity Type</code> only.
              Any entity type value is accepted.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="flex items-center gap-3">
              <Label
                htmlFor="status-import-file-input"
            ...
            {preview && (
              <div className="rounded-md border p-3 space-y-2 text-sm">
                ...
                {preview.valid_rows === 0 && preview.errors.length === 0 && (
                  <p className="text-muted-foreground">
                    No new statuses to import{preview.skipped > 0 ? " — all rows already exist." : "."}
                  </p>
                )}
              </div>
            )}
